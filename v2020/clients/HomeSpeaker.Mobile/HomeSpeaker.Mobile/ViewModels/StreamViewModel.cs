...
namespace HomeSpeaker.Mobile.ViewModels
{
    public class StreamViewModel : BaseViewModel
    {
        ...
        public Command<string> PlayStream => playStream ?? (playStream = new Command<string>(async (path) =>
        {
            await client.PlayStreamAsync(new Server.gRPC.PlayStreamRequest { StreamUrl = path });
        }));
    }
}